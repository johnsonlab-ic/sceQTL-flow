...
#### HTML Report:
- `{outdir}/eQTL_outputs/eqtl_report.html` — Main eQTL report
- (New section for PC optimization with visualizations)
...
No changes required to user parameters. The visualization is automatically generated when:
1. `--optimize_pcs true` (PC optimization enabled)
2. `--report true` (reporting enabled)

To generate the PC optimization report standalone:
```bash
cd R/quarto_reports
OUTDIR=/path/to/results quarto render pc_optimization_report.qmd
```
...
- ✅ Report section automatically includes/excludes based on pipeline mode
- ✅ No additional parameters required (uses existing `--optimize_pcs` flag)
