...
if (!is.null(eqtl_results) && length(eqtl_results) > 0) {
  ...
  full_eqtl_dt <- purrr::imap_dfr(eqtl_filtered_dynamic, function(df, ct) {
    ...
    tibble(
      ...
      effect_allele = if ("effect_allele" %in% colnames(df)) as.character(df$effect_allele) else NA_character_,
      other_allele = if ("other_allele" %in% colnames(df)) as.character(df$other_allele) else NA_character_,
      p.value = formatC(as.numeric(df$p.value), format = "e", digits = 2),
      FDR = formatC(as.numeric(df$FDR), format = "e", digits = 2),
      beta = as.character(signif(as.numeric(df$beta), 3)),
      se = as.character(signif(as.numeric(se_vals), 3))
    )
  })
  ...
  if (nrow(full_eqtl_dt) > 0) {
    DT::datatable(
      ...
      caption = htmltools::tags$caption(
        ...
        paste0("Full eQTL table by cell type (FDR <= ", fdr_cutoff, ")")
      )
    )
  } else {
