...
if (!is.null(eqtl_results) && length(eqtl_results) > 0) {
  ...
  unique_egenes_long <- purrr::imap_dfr(eqtl_filtered_dynamic, function(df, ct) {
    if (!"gene" %in% colnames(df) || nrow(df) == 0) {
      return(tibble(cell_type = gsub("_", " ", ct), gene = character(0)))
    }
    tibble(
      cell_type = gsub("_", " ", ct),
      gene = sort(unique(as.character(df$gene)))
    )
  })

  if (nrow(unique_egenes_long) > 0) {
    DT::datatable(
      unique_egenes_long,
      rownames = FALSE,
      filter = "top",
      ...
      caption = htmltools::tags$caption(
        ...
        paste0("Unique eGenes by cell type (FDR <= ", fdr_cutoff, ")")
      )
    )
  } else {
    cat("No eGenes passed the configured FDR threshold for DT summary.\n")
  }
} else {
