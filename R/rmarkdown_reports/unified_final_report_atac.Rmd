---
title: "Unified ATAC-QTL & PC Optimization Report"
...
data_type <- if (!is.null(params$data_type) && nzchar(params$data_type)) toupper(params$data_type) else "ATAC"
...
# Exclude placeholder cell-type labels from all report sections.
is_excluded_celltype <- function(celltype_values) {
  normalized <- trimws(as.character(celltype_values))
  normalized[is.na(normalized)] <- ""
  normalized <- tolower(normalized)
  normalized <- gsub("[[:space:]_-]+", "", normalized)
  grepl("^(unknown|unassigned|na|n/a)$", normalized)
}

filter_celltype_named_list <- function(x) {
  if (is.null(x) || length(x) == 0 || is.null(names(x))) {
    return(x)
  }
  x[!is_excluded_celltype(names(x))]
}

filter_celltype_df <- function(df, column_name = "celltype") {
  if (is.null(df) || !column_name %in% colnames(df)) {
    return(df)
  }
  dplyr::filter(df, !is_excluded_celltype(.data[[column_name]]))
}
...
## ATAC-QTL Analysis Report

**Report Generated:** `r format(Sys.time(), "%d %B %Y, %H:%M:%S")`

This report was generated using the Nextflow ATAC-QTL Pipeline.
...
eqtl_results_filtered <- filter_celltype_named_list(eqtl_results_filtered)
...
eqtl_results <- filter_celltype_named_list(eqtl_results)

# Load peaks tested summary
...
genes_tested_df <- filter_celltype_df(genes_tested_df, "celltype")
...
individuals_df <- filter_celltype_df(individuals_df, "celltype")
if (!is.null(individuals_df) && nrow(individuals_df) == 0) {
  individuals_df <- NULL
}
...
- **Average number of peaks tested per cell-type:** `r avg_genes_tested_display`
...
if (!is.null(eqtl_results_filtered)) {
  ...
  g <- ggplot(df_long, aes(x = celltype, y = count, fill = celltype_col)) +
    ...
    labs(y = "Total vs unique ePeaks (5% FDR)") +
  ...
  cat("Filtered ATAC-QTL results not available for ePeak plot.\n")
}
```

### total ePeaks vs total peaks tested

```{r plot-epeaks-vs-tested, echo=FALSE, fig.width=10, fig.height=5}
if (!is.null(eqtl_results_filtered) && !is.null(genes_tested_df) && nrow(genes_tested_df) > 0) {
  epeaks_discovered_df <- tibble(
    celltype = names(eqtl_results_filtered),
    epeaks_discovered = vapply(eqtl_results_filtered, function(df) dplyr::n_distinct(df$gene), numeric(1))
  )

  plot_df <- epeaks_discovered_df %>%
  ...
  plot_long <- plot_df %>%
    pivot_longer(
      cols = c(epeaks_discovered, genes_tested),
      names_to = "metric",
      values_to = "n_peaks"
    ) %>%
    mutate(
      metric = dplyr::recode(metric,
        epeaks_discovered = "ePeaks discovered",
        genes_tested = "Total peaks tested"
      ),
      celltype = factor(celltype, levels = unique(plot_df$celltype))
    )

  ggplot(plot_long, aes(x = celltype, y = n_peaks, group = metric, color = metric)) +
    geom_line(size = 0.8) +
    geom_point(size = 2) +
    labs(x = "Cell Type", y = "Number of peaks", color = "Metric") +
    theme_minimal() +
    theme(
      panel.grid.minor = element_blank(),
      axis.text.x = element_text(angle = 45, hjust = 1),
  ...
  cat("Peaks tested summary is unavailable; cannot plot ePeaks discovered vs total peaks tested.\n")
}
```

### ATAC-QTL Summary Tables (Filtered at Configured FDR)
...
if (!is.null(eqtl_results) && length(eqtl_results) > 0) {
  ...
  summary_tbl <- tibble(
    ...
    total_unique_ePeaks = vapply(eqtl_filtered_dynamic, function(df) dplyr::n_distinct(df$gene), numeric(1))
  ) %>%
    mutate(cell_type = gsub("_", " ", cell_type)) %>%
    arrange(desc(total_unique_ePeaks))

  knitr::kable(
    summary_tbl,
    col.names = c("Cell Type", "Total Unique ePeaks"),
    caption = paste0("Unique ePeaks per cell type at FDR <= ", fdr_cutoff)
  )
  ...
  full_eqtl_dt <- purrr::imap_dfr(eqtl_filtered_dynamic, function(df, ct) {
    ...
    peak_values <- as.character(df$gene)
    ...
    tibble(
      cell_type = gsub("_", " ", ct),
      SNP = top_per_peak$SNP,
      peak = top_per_peak$peak,
      effect_allele = top_per_peak$effect_allele,
      other_allele = top_per_peak$other_allele,
      p.value = formatC(top_per_peak$p_numeric, format = "e", digits = 2),
      FDR = formatC(top_per_peak$fdr_numeric, format = "e", digits = 2),
      beta = as.character(signif(top_per_peak$beta_numeric, 3)),
      se = as.character(signif(top_per_peak$se_numeric, 3))
    )
  })
  ...
  if (nrow(full_eqtl_dt) > 0) {
    DT::datatable(
      ...
      caption = htmltools::tags$caption(
        ...
        paste0("Unique-peak ATAC-QTL table by cell type (best association per peak, FDR <= ", fdr_cutoff, ")")
      )
    )
  } else {
    cat("No ATAC-QTL associations passed the configured FDR threshold for DT summary.\n")
  }
} else {
  cat("ATAC-QTL results are unavailable; cannot build summary tables.\n")
}
```
...
This section details the two-stage PC optimization process used in the pipeline.
...
coarse_data <- filter_celltype_df(coarse_data, "celltype")
if (!is.null(coarse_data) && nrow(coarse_data) == 0) {
  coarse_data <- NULL
}
```

The coarse grid evaluates a broad range of PC values in large steps (default step size = 10) to quickly identify the region with the best QTL power. Early stopping halts the search when two consecutive steps show less than 1% improvement in associations, preventing unnecessary computation. The plots below show the total number of associations and percent gains at each PC value tested, with the best PC (selected for fine-tuning) highlighted in red.
...
fine_data <- filter_celltype_df(fine_data, "celltype")
if (!is.null(fine_data) && nrow(fine_data) == 0) {
  fine_data <- NULL
}
```
...
if (!is.null(fine_data)) {
  ...
  fine_data %>%
    ...
    scale_color_manual(values = plot_colors, guide = "none") +
    facet_wrap(~celltype, scales = "free") +
    labs(x = "Number of PCs", y = "Number of ePeaks") +
    theme_minimal() +
    theme(
      panel.grid.minor = element_blank(),
      strip.text = element_text(size = 10, face = "bold"),
...
- **Interpretation**: This selection avoids overfitting (too many PCs) while capturing nearly-optimal QTL power
