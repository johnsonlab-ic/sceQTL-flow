...
| **Final Report** | `modules/reports/final_report.nf` | Render unified HTML report (eQTL + PC optimization results) |
...
| `--cov_file` | Covariate file (optional) | `covariates.csv` |
| `--subset_column` | Column to subset on | `"Diagnosis"` |
| `--subset_values` | Values to keep | `"Control,AD"` |
...
| `--report` | Render unified final HTML report | `true` |
...
│   ├── quarto_reports/           # Quarto report templates
│   │   └── pc_optimization_report.qmd
│   └── rmarkdown_reports/        # R Markdown report templates
│       └── unified_final_report.Rmd  # Unified eQTL + PC optimization report
...
| File | Description |
|------|-------------|
| `eqtl_report.html` | Unified eQTL + PC optimization report |
| `mateqtlouts.rds` | Combined eQTL results (all cell types) |
| `mateqtlouts_FDR_filtered.rds` | FDR-filtered eQTL results |
| `{celltype}_coarse_summary.csv` | Coarse PC optimization results |
| `{celltype}_fine_summary.csv` | Fine PC optimization results |
